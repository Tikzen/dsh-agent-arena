...
function isArenaRateLimitFailure(value) {
	const failure = value?.failure && typeof value.failure === "object" ? value.failure : value;
	return Number(failure?.status) === 429 || RATE_LIMIT_RE.test(`${String(failure?.code || "")} ${String(failure?.message || safeError(value))}`);
}
...
//#endregion
export { apply, arenaChannelKey, src_default as default, inject, installArenaModelSelection, isArenaRateLimitFailure };
